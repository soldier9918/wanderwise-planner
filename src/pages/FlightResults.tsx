...
import {
  ArrowRight, Plane, Clock, AlertCircle, Loader2, ChevronDown,
  Globe, Sparkles, Search, SlidersHorizontal, X, ChevronUp,
} from "lucide-react";
...
// ── Helpers ────────────────────────────────────────────────────────────────────
function toSkyscannerDate(d: string) { const [y,m,dd]=d.split("-"); return y.slice(2)+m+dd; }
...
function AirlineLogo({ code, name }: { code: string; name: string }) {
  ...
  if (err) return (
    <div className="w-10 h-10 rounded-xl bg-secondary flex items-center justify-center text-[10px] font-bold text-muted-foreground border border-border shrink-0">
      {code.slice(0, 2)}
    </div>
  );
  return (
    <img src={airlineLogoUrl(code)} alt={name}
      className="w-10 h-10 rounded-xl object-contain bg-card p-1 border border-border shrink-0"
      onError={() => setErr(true)} />
  );
}
  ...
  return (
    <div className="flex items-center gap-3 flex-1 min-w-0">
      <div className="text-center shrink-0 w-14">
        <p className="text-xl font-bold text-foreground leading-none">{formatTime(first.departure.at)}</p>
        <p className="text-xs text-muted-foreground mt-0.5 font-medium">{first.departure.iataCode}</p>
      </div>
        ...
        <div className="relative w-full flex items-center h-4">
          ...
          <div className="absolute inset-0 flex items-center justify-center">
            {stops === 0 ? (
              ...
              <div className="bg-card px-1 flex gap-1">
                {Array.from({ length: stops }).map((_, i) => (
                  <div key={i} className="w-2 h-2 rounded-full border-2 border-warning bg-card" />
                ))}
              </div>
            )}
          </div>
          <div className="absolute right-0 text-muted-foreground/40">▶</div>
        </div>
        <p className={`text-xs font-semibold leading-none ${stops === 0 ? "text-success" : "text-warning"}`}>
          {stops === 0 ? "Direct" : `${stops} stop${stops > 1 ? "s" : ""}`}
        </p>
      </div>
      <div className="text-center shrink-0 w-14">
        <p className="text-xl font-bold text-foreground leading-none">{formatTime(last.arrival.at)}</p>
        <p className="text-xs text-muted-foreground mt-0.5 font-medium">{last.arrival.iataCode}</p>
      </div>
    </div>
  );
}
    ...
    >
      <div className="flex items-center gap-4 p-5 cursor-pointer" onClick={onToggle}>
        ...
        <div className="flex flex-col items-center gap-1.5 shrink-0 w-16 text-center">
          ...
          <p className="text-[10px] text-muted-foreground leading-tight line-clamp-2 w-full">{airline}</p>
        </div>
        ...
        {/* Price */}
        <div className="shrink-0 flex flex-col items-end gap-2 ml-2 min-w-[120px]">
          <div className="text-right">
            <p className="text-2xl font-bold text-foreground">£{priceGBP.toFixed(0)}</p>
          ...
          <button
            ...
            className={`w-full flex items-center justify-center gap-1.5 px-5 py-2.5 rounded-full text-sm font-bold transition-all ${
              isExpanded ? "bg-primary/10 text-primary border border-primary/30" : "bg-primary text-primary-foreground hover:bg-primary/90"
            }`}
          >
...
function FilterSection({ title, children, defaultOpen = true }: { title: string; children: React.ReactNode; defaultOpen?: boolean }) {
  ...
  return (
    <div className="border-b border-border py-4 last:border-0">
      <button onClick={() => setOpen(!open)} className="flex items-center justify-between w-full mb-0 text-left group">
...
// ── Time slider ────────────────────────────────────────────────────────────────
function TimeRangeLabel({ minutes }: { minutes: number }) {
...
interface FilterState {
  stops: { direct: boolean; one: boolean; twoPlus: boolean };
  maxPrice: number;
  outboundTime: [number, number]; // minutes from midnight
  returnTime: [number, number];
...
function FilterSidebar({ filters, setFilters, priceRange, durationRange, airlineOptions, onClose }: FilterSidebarProps) {
  ...
  return (
    <div className="bg-card border border-border rounded-2xl overflow-hidden">
      ...
      <div className="flex items-center justify-between px-5 py-4 border-b border-border">
        <h2 className="font-bold text-foreground text-sm">Filters</h2>
        {onClose && (
        ...
        <FilterSection title="Stops">
          <div className="space-y-0.5">
            <CheckRow
              ...
              onChange={(v) => set("stops", { ...filters.stops, direct: v })}
              label="Direct"
            />
            <CheckRow
              checked={filters.stops.one}
              onChange={(v) => set("stops", { ...filters.stops, one: v })}
              label="1 stop"
            />
            <CheckRow
              checked={filters.stops.twoPlus}
              onChange={(v) => set("stops", { ...filters.stops, twoPlus: v })}
              label="2+ stops"
            />
          </div>
        </FilterSection>
        ...
        <FilterSection title="Departure times" defaultOpen={false}>
          <div className="space-y-4">
            <div>
              ...
              <p className="text-xs text-muted-foreground mb-2">
                <TimeRangeLabel minutes={filters.outboundTime[0]} /> – <TimeRangeLabel minutes={filters.outboundTime[1]} />
              </p>
              <input
                type="range" min={0} max={1439}
                value={filters.outboundTime[1]}
                onChange={(e) => set("outboundTime", [filters.outboundTime[0], Number(e.target.value)])}
                className="w-full accent-primary h-1.5"
              />
            </div>
            <div>
              ...
              <p className="text-xs text-muted-foreground mb-2">
                <TimeRangeLabel minutes={filters.returnTime[0]} /> – <TimeRangeLabel minutes={filters.returnTime[1]} />
              </p>
              <input
                type="range" min={0} max={1439}
                value={filters.returnTime[1]}
                onChange={(e) => set("returnTime", [filters.returnTime[0], Number(e.target.value)])}
                className="w-full accent-primary h-1.5"
              />
            </div>
          </div>
...
const FlightResults = () => {
  ...
  const [filters, setFilters] = useState<FilterState>({
    stops: { direct: true, one: true, twoPlus: true },
    maxPrice: 99999,
    outboundTime: [0, 1439],
    returnTime: [0, 1439],
    maxDuration: 99999,
    ...
    const fetch_ = async () => {
      ...
      try {
        const { data, error: fnError } = await supabase.functions.invoke("amadeus-flight-search", {
          body: { originLocationCode: from.toUpperCase(), destinationLocationCode: to.toUpperCase(),
            departureDate: depart, returnDate: returnDate || undefined, adults, children,
            travelClass: cabin.toUpperCase().replace(" ", "_"), nonStop: direct, currencyCode: "GBP", max: 30 },
        });
        ...
        // Initialise filter ranges from data
        if (results.length > 0) {
  ...
  const { priceRange, durationRange, airlineOptions } = useMemo(() => {
    if (flights.length === 0) return { priceRange: [0, 9999] as [number,number], durationRange: [0, 1440] as [number,number], airlineOptions: [] };
    ...
    const airlineOpts = Object.entries(airlineMin)
  ...
  const sorted = useMemo(() => {
    return [...flights]
      .filter((f) => {
        ...
        if (stops === 0 && !filters.stops.direct) return false;
        ...
        const dur = parseDurationMinutes(f.itineraries[0].duration);
        if (dur > filters.maxDuration) return false;
        const outMins = timeToMinutes(f.itineraries[0].segments[0].departure.at);
        if (outMins < filters.outboundTime[0] || outMins > filters.outboundTime[1]) return false;
        const airlineCode = f.validatingAirlineCodes[0];
        if (filters.airlines[airlineCode] === false) return false;
        return true;
      })
      .sort((a, b) => {
      ...
      <div className="pt-16">
        ...
        <div className="bg-card border-b border-border sticky top-16 z-30 shadow-sm">
          <div className="container mx-auto px-4 py-3 flex items-center gap-2 flex-wrap">
            ...
            {depart && (
              <div className="flex items-center gap-1.5 bg-secondary rounded-xl px-4 py-2 text-sm text-foreground">
                <Clock className="w-3.5 h-3.5 text-muted-foreground" />
                <span>{formatDate(depart)}</span>
                {returnDate && <><span className="text-muted-foreground">→</span><span>{formatDate(returnDate)}</span></>}
              </div>
            )}
          ...
          <div className="bg-card border-b border-border">
            <div className="container mx-auto px-4">
              <div className="flex items-stretch gap-0">
                {([
                  { key: "price" as const, label: "Cheapest", sublabel: cheapest ? `£${cheapest}` : "–" },
                  { key: "duration" as const, label: "Fastest", sublabel: fastest ? `£${fastest}` : "–" },
                  { key: "stops" as const, label: "Fewest stops", sublabel: "" },
                ] as const).map((opt) => (
                  <button
                    key={opt.key}
                  ...
                  >
                    <p className={`text-xs font-semibold ${sortBy===opt.key?"text-muted-foreground":"text-muted-foreground"}`}>{opt.label}</p>
                    {opt.sublabel && <p className={`text-lg font-bold ${sortBy===opt.key?"text-primary":"text-foreground"}`}>{opt.sublabel}</p>}
                  </button>
                ))}
                {/* Mobile filter button */}
                <button
                  onClick={() => setShowMobileFilters(true)}
                  className="flex items-center gap-2 px-5 py-4 border-b-2 border-transparent text-sm font-semibold text-foreground hover:bg-secondary/60 transition-colors lg:hidden"
                >
              ...
              {!loading && !error && sorted.length === 0 && (
                <div className="flex flex-col items-center justify-center py-24 gap-4 text-center">
                  ...
                  <p className="text-muted-foreground text-sm max-w-sm">
                    Try adjusting the filters or searching different dates.
                  </p>
                </div>
              )}
              ...
              {!loading && !error && sorted.length > 0 && (
                <>
                  <p className="text-sm text-muted-foreground mb-4 font-medium">
                    {sorted.length} result{sorted.length!==1?"s":""} sorted by {sortBy}
                  </p>
                  <div className="space-y-2.5">
                    {sorted.map((offer, i) => (
                      <FlightCard
                        key={offer.id} offer={offer} index={i}
